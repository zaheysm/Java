...
public class Inventory {
     ...
     */
    public int recursiveBinarySearch(int itemCode, ArrayList<FoodItem> itemArry, int first, int last) {
        if (first <= last) {      //condition first smaller than last
            int mid = first + (last - first) / 2;     //the new start of the search
            if (itemArry.get(mid).getItemCode() < itemCode) {     //statement if array in place of smaller than key we are searching
